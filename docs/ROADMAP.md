...
- [x] Не коммитить автоматически из Codex.
- [x] Каждый пункт с `Commit:` коммитит пользователь вручную после проверки.
- [x] Держать версию приложения `1.0.0` с первого рабочего build.
- [x] Не делать MVP-режим: каждая стадия должна оставлять проект в аккуратном, собираемом состоянии.
- [x] Не смешивать большие независимые изменения в один коммит.
- [x] Любой системный API оборачивать в C-слой или тонкий adapter, если это возможно без грязной архитектуры.
- [x] Все persistent-данные хранить через SQLite.
- [x] Все небезопасные macOS-ограничения явно отражать в UI и логике, а не притворяться, что можно переключать то, что macOS системно не дает переключать.
...
- [x] Показать список профилей.
- [x] Добавить создание профиля.
- [x] Добавить переименование профиля.
- [x] Добавить редактирование профиля.
- [x] Добавить удаление профиля.
- [x] Добавить выбор устройств по ролям.
- [x] Для каждой роли показывать только подходящие устройства.
- [x] Позволить сохранить missing device, если профиль был создан раньше, а устройство сейчас отключено.
- [x] Добавить кнопку `Activate`.
- [x] Показывать активный профиль.
- [x] Не перезаписывать профиль при manual switch.
...
- [x] Проверить обнаружение встроенного микрофона и динамиков.
- [x] Проверить подключение USB-микрофона.
- [x] Проверить подключение Bluetooth-наушников.
- [x] Проверить сценарий сброса default input после подключения Bluetooth-наушников.
- [x] Проверить, что приложение возвращает выбранный USB-микрофон.
- [x] Проверить отключение выбранного устройства.
- [x] Проверить повторное подключение выбранного устройства.
- [x] Проверить работу после sleep/wake.
- [x] Проверить работу после закрытия окна.
- [x] Проверить Quit из menu bar.

Статус проверки: подтвержденная автоматизированная часть, source-level checks, C-core simulation и список ручного hardware-retest перед публикацией записаны в `docs/MACOS_INTEGRATION_CHECKS.md`.
...
- [x] Проверить, что app version `1.0.0`.
- [x] Проверить, что build number корректный.
- [x] Проверить, что приложение называется `MacPeripheralHub`.
- [x] Проверить, что bundle содержит app icon.
- [x] Проверить, что закрытие окна убирает Dock icon.
- [x] Проверить, что menu bar остается живым.
- [x] Проверить, что Quit полностью завершает процесс.
- [x] Проверить, что login item prompt не повторяется бесконечно.
- [x] Проверить, что C-кода в репозитории больше, чем Swift-кода.
- [x] Проверить, что `make test-core` проходит.
...
- [x] `test(app): verify macOS device switching flows`
